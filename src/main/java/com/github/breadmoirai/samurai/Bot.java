...
public class Bot {
    ...
    private static void start(String[] args) {
        ...
        BreadBotClient breadBotClient = new BreadBotClientBuilder()
                .buildAnnotated();


        EventWaiter.get().waitForEvent(ReadyEvent.class, e -> true, e -> info = new BotInfo(e.getJDA()));

        final JDABuilder jdaBuilder = new JDABuilder(AccountType.BOT)
                .setToken(ConfigFactory.load().getString("bot.token"))
                .setAudioEnabled(true)
                .setEventManager(breadBotClient.getEventManager())
...
//        shards.stream()
//                .map(jda -> (JDAImpl) jda)
//                .filter(jda -> jda.getShardInfo() == null
//                        || jda.getShardInfo().getShardId() != 0)
//                .forEach(jda -> jda.getEventManager().handle(event));
//    }
