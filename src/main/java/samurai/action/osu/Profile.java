...
public class Profile extends Action {
    ...
    protected SamuraiMessage buildMessage() {
        ...
        if (args.size() == 0 && mentions.size() == 0) {
            ...
            long userId = Long.parseLong(mentions.get(0).getId());
            if (guild.hasUser(userId)) {
                ...
                return FixedMessage.build(String.format("**%1$s** has not yet linked their osu profile. Try using `!link @%1$s [Osu!Username]`", mentions.get(0).getName()));
        } else {
